...
class OtherEventRecord:
    """Метод для работы с записями журнала других событий"""
    METHOD = 'journal/other_event/'
    ...
    async def create(self, name, description, dt):
        request_url = self.METHOD_URL
        data = dict(name=name, description=description, dt=dt)
        async with self.session.post(request_url, data=data, headers=self.headers) as response:
            response.raise_for_status()
            return await response.json()

    async def update(self, record_id, name, description, dt):
        request_url = self.METHOD_URL + record_id
        data = dict(name=name, description=description, dt=dt)
