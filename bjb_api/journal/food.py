...
class FoodRecord:
    """Метод для работы с записями журнала питания"""
    METHOD = 'journal/food/'
    ...
    async def create(self, category, quantity, dt, name=None):
        request_url = self.METHOD_URL
        data = dict(category=category, name=name, quantity=quantity, dt=dt)
        async with self.session.post(request_url, data=data, headers=self.headers) as response:
            print(response.__dict__)
            response.raise_for_status()
            return await response.json()

    async def update(self, record_id, category, name, quantity, dt):
        request_url = self.METHOD_URL + record_id
        data = dict(category=category, name=name, quantity=quantity, dt=dt)
